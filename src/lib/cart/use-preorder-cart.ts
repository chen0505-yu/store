...
// 商品架構改為 老師 → 品項（product_groups） → 細項（product_variants）後，
// 預購購物車以「細項」為最小購買單位，variantId 全域唯一（uuid），可以直接當作合併鍵。
// 跟現貨購物車（use-instock-cart.ts）完全分開的 store，資料形狀也不同，互不影響。
...
interface PreorderCartState {
  ...
  addItem: (item: PreorderCartItem) => void;
