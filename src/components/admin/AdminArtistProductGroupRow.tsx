...
import type { ArtistProductGroup, ArtistVariant } from "@/lib/data/artist-shop";
import type { Tag } from "@/lib/data/tags";
import type { ArrivalStatus } from "@/lib/types";
import {
  addArtistVariant,
  archiveArtistProductGroup,
  renameArtistVariant,
  setArtistBlindDrawConfig,
  setArtistGroupArrivalStatus,
  setArtistGroupSurcharge,
  setArtistVariantSurcharge,
  toggleArtistGroupCpSpoiler,
  toggleArtistVariantActive,
  toggleArtistVariantBonusOption,
  updateArtistProductGroup,
...
function VariantRow({ variant }: { variant: ArtistVariant }) {
  ...
  function handleSave() {
    startTransition(async () => {
      await renameArtistVariant(variant.id, name);
      setEditing(false);
    });
  }
  ...
  function handleSaveSurcharge() {
    startTransition(async () => {
      const result = await setArtistVariantSurcharge(
        variant.id,
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() =>
              startTransition(async () => {
                await toggleArtistVariantActive(variant.id, !variant.isActive);
              })
            }
            disabled={isPending}
          ...
          <button
            onClick={() =>
              startTransition(async () => {
                await toggleArtistVariantBonusOption(variant.id, !variant.isBonusOption);
              })
            }
            disabled={isPending}
...
export function AdminArtistProductGroupRow({ group, allTags }: { group: ArtistProductGroup; allTags: Tag[] }) {
  const [editing, setEditing] = useState(false);
  ...
  function handleSaveEdit() {
    startTransition(async () => {
      const result = await updateArtistProductGroup(group.id, {
        name,
  ...
  function handleSaveBlindDraw() {
    startTransition(async () => {
      const result = await setArtistBlindDrawConfig(group.id, {
        isBlindDraw,
  ...
  function handleSaveSurcharge() {
    startTransition(async () => {
      const result = await setArtistGroupSurcharge(
        group.id,
        surchargeAmount.trim() === "" ? null : Number(surchargeAmount),
        surchargeReason
      );
  ...
  function handleAddVariant() {
    ...
    startTransition(async () => {
      const result = await addArtistVariant(group.id, newVariantName);
    ...
    <div className="flex flex-col gap-3 rounded-2xl bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <select
            ...
            onChange={(e) =>
              startTransition(async () => {
                await setArtistGroupArrivalStatus(group.id, e.target.value as ArrivalStatus);
              })
            }
            disabled={isPending}
          ...
          <button
            onClick={() =>
              startTransition(async () => {
                await toggleArtistGroupCpSpoiler(group.id, !group.isCpSpoiler);
              })
            }
            disabled={isPending}
          ...
          <button
            onClick={() => {
              ...
              startTransition(async () => {
                await archiveArtistProductGroup(group.id);
              });
            }}
            disabled={isPending}
      ...
      <div className="flex flex-col gap-2 rounded-xl bg-pink-50/60 p-3">
        <label className="flex items-center gap-2 text-xs font-semibold text-pink-700">
          <input type="checkbox" checked={isBlindDraw} onChange={(e) => setIsBlindDraw(e.target.checked)} />
      ...
      <div className="flex flex-col gap-1 rounded-xl bg-zinc-50 p-3">
        <p className="text-xs font-semibold text-zinc-500">細項（盲抽開啟時，可以在每個細項上標記「可選保底」）</p>
        <div className="flex flex-col gap-1">
