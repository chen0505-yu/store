...
import type { ArtistShopFront, ArtistShopGroup } from "@/lib/data/artist-storefront";
...
// CP 防雷：跟 TeacherShopView 的 GroupImage 邏輯完全相同，只是型別換成繪師賣場的品項。
function GroupImage({ group }: { group: ArtistShopGroup }) {
  ...
  return <ImageGalleryLightbox images={images} alt={group.name} thumbnailClassName="h-24 w-24 rounded-xl object-cover" />;
}
...
export function ArtistShopView({ shop, isBlacklisted }: { shop: ArtistShopFront; isBlacklisted: boolean }) {
  const { items, addItem, updateQuantity, removeItem } = useArtistCart();
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-4 pt-8 pb-[180px] md:pb-8">
      <Link href="/artist" className="text-sm text-purple-500 underline">
        ← 回繪師預購專區
      </Link>
      ...
      <div className="mt-4 flex items-center gap-4 rounded-3xl bg-white p-5 shadow-sm">
        {coverImage ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={coverImage} alt={shop.teacherName} className="h-16 w-16 rounded-full object-cover" />
        ) : (
          <div className="flex h-16 w-16 items-center justify-center rounded-full bg-purple-50 text-2xl">🦝</div>
        )}
        <div>
          <h1 className="text-xl font-bold text-zinc-800">{shop.teacherName}</h1>
          {shop.socialUrl && (
            <a href={shop.socialUrl} target="_blank" rel="noreferrer" className="text-xs text-purple-500 underline">
              社群連結
            </a>
          )}
          {(startText || endText) && (
          ...
          {phase === "not_started" && <p className="mt-1 text-xs font-medium text-orange-500">預購尚未開始</p>}
      ...
      {conflictMessage && (
        <div className="mt-4 rounded-2xl bg-red-50 p-3 text-sm font-semibold text-red-600">{conflictMessage}</div>
      )}

      {shop.groups.length === 0 ? (
        <p className="mt-6 text-center text-sm text-zinc-500">這位繪師目前沒有預購品項。</p>
      ) : (
                    ...
                    .map((variant) => {
                      ...
                      function setQuantity(next: number) {
                        ...
                        if (next <= 0) {
                          ...
                          if (!added) {
                            setConflictMessage("購物車一次只能放一位繪師的商品，請先結清或清空購物車。");
                          }
                        }
                      }
      ...
      <FloatingCartButton cartType="artist" />
    </div>
  );
}
