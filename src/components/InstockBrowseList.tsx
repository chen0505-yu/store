...
import type { InstockTeacherSummary } from "@/lib/data/instock-shop";
import { EmptyState } from "@/components/EmptyState";

// 現貨專區依老師分組瀏覽：每張卡片是一位老師的現貨賣場，點進去就是「老師現貨賣場頁」，
// 賣場底下的品項/細項集中在同一頁，不用每個商品各自獨立成一頁分散瀏覽。
export function InstockBrowseList({ teachers }: { teachers: InstockTeacherSummary[] }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {filtered.length === 0 ? (
        <EmptyState text="找不到符合的老師現貨賣場" />
      ) : (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
          {filtered.map((teacher) => (
            <Link
              key={teacher.teacherId}
              href={`/instock/teacher/${teacher.teacherId}`}
